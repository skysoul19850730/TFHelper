...
class ExcelUtil {
    ...
    fun mtest(){
        val work = HSSFWorkbook(File(Config.caiji_main_path,"test.xls").inputStream())
    ...
    fun test(){
        ...
        val work = XSSFWorkbook(File(Config.caiji_main_path,"test.xlsx").inputStream())
        val sheet = work.getSheet("Sheet1")
        ...
        val out = File(Config.caiji_main_path,"test.xlsx").outputStream()
    ...
    private fun writeOneUser(user:File,sheet:Sheet,modelRow:Row){
        ...
        var row: Row? = sheet.firstOrNull {
            it.getCell(0).stringCellValue == user.name
        }
        if(row==null){
            row = sheet.createRow(sheet.lastRowNum+1).apply {
                createCell(0).setCellValue(user.name)
            }
        }
        ...
        for(line2 in lines){
            val line =line2.replace("\\s+".toRegex()," ")
