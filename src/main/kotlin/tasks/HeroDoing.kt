...
//            hs = getPreHeros(if (needShuaxin) 200 else 10000)//点完刷新如果1秒中识别不出来应该就是识别不出来了，这里不需要2秒
            ...
            if (!needShuaxin) {
                ...
                if (needCheckQian) {
                    //这里其实不应该用  有红色 来判断，一个是可能有弹窗下卡和这里的逻辑并行，那就不会发现红色，而认为能刷卡了
                    //另一个就是可能有的设备或同设备本次游戏的界面，它不是纯纯的红色，我记得公司电脑上打对战时，它就会明明红色但一直点刷新
                    measureTimeMillis {
                        while (Config.rect4ShuakaColor.hasColor(Color.RED)) {//有白色（钱够）再点击刷新->改成如果有红色
                            delay(50)
                        }
...
//                ))
    ...
    private suspend fun clickShuaxinAndWaitYuxuanChanged3(): List<HeroBean?>? {
        ...
        while (shuaxinClicked && running) {
            ...
            MRobot.singleClick(Config.zhandou_shuaxinPoint)
            ths = null //这里要重新赋值null//除了下卡，比如第一次点刷新确实cd没点出，但此时有上次的预选卡，使用旧方法再次识别时（这里是后加的逻辑，所以引起了这个问题）
            //ths就有值了，且就还是上次遗留的预选卡，但因为一样所以认为没刷新成功，这时又点刷新，这次会遇到白了，本来只有新方法时，这个ths一直都是null的
            //是识别到白，就走一次识别（认为刷新点成功了就是识别了），但这个ths这次不是null，所以就没有去识别白之后的，也就是卡已经刷了，但按上次的卡进行的deal和上卡
            //就会上错，比如要上王将结果上了海妖，后续想上王将就会报已经没有位置了（大漩涡70的bug）因为大部分都是上完卡，没有预选卡的情况下，这个情况不会发生。
            //但漩涡69是要下卡补卡的，为了快速不卡，光球不验星星，而且没有上满就over的逻辑，上满也会刷出一组卡，再下次deal时通过delay卡住，所以就会存在预选卡
            //包括幻装备后也会有上次的卡。这种通过delay卡住，且有预选卡和下卡的情况都会遇到上错上一组卡的情况，下卡是导致的第一次点刷新失败了，没点中根本，下卡弹窗消失后露出了之前卡组
            //然后使用旧方法识别了一样的，导致认为没刷新，然后又点刷新成功了，卡刷走了，但没执行后面的getheros(因为ths不是null）
            //即使没有下卡，也可能点刷新不成功出现上面一样的问题（不过这种比较难见，刷新点了不成功要么是没挡住了，确实没点中，要么是游戏自己刷新的点击cd，但是点了却无效，但这种一般出现
            //在连续点刷新上卡的情况，想大漩涡到了新关卡，才刷新，基本不会出）另外既然认为没刷新成功，那点刷新后把ths置空，本来就是合理行为
            //补：还有，比如有的电脑上判断刷新钱币颜色那里有问题，明明红色的还一直点，比如对战。刷了一组卡都不要，然后点刷但没刷到也会认为和上一组一样
            //等有钱了突然刷出白色认为刷新了，结果就会直接返回上一组的卡，会认为还是不要，就又去点刷新，这就是偶尔遇到的 明明这次刷出的卡是要的，为啥没上去，甚至日志里都没有刚新刷出的卡组
            //因为根本没走getheros。
