...
class SHZJHeroDoingZQ : BaseSimpleSHHeroDoing() {
    ...
    override fun initHeroes() {
        super.initHeroes()
        heros = arrayListOf(tieqi, zhanjiang, yuren, feiting, tuling, sishen, moqiu, hunqiu, guangqiu, gugu)

        addGuanDeal(0) {
            over {
                fulls(zhanjiang, sishen, gugu, feiting)
            }
            chooseHero {
                if (zhanjiang.isInCar()) {
                    upAny(zhanjiang, sishen, gugu, feiting)
                } else upAny(zhanjiang)
            }
        }

        addGuanDealWithHerosFull(28, listOf(zhanjiang, sishen, feiting, gugu, tieqi, tuling, yuren))
