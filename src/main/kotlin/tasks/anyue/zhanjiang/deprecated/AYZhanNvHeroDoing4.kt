...
class AYZhanNvHeroDoing4 : BaseAnYueHeroDoing(),
...
//                }
            ...
            Guan.g21 -> {
                dijing.isFull() && jiaonv.isFull() && niutou.isFull() && nvwang.isFull() && saman.isFull() && baoku.isFull() && Zhuangbei.isYandou()
            }

            Guan.g28 -> shahuang.isFull() && jiaonv.isFull() && niutou.isFull() && nvwang.isFull() && saman.isFull() && baoku.isFull() && Zhuangbei.isYandou()
            Guan.g31 -> shahuang.isFull()
//            Guan.g31 -> Zhuangbei.isShengjian() && shahuang.isFull()  && baoku.isFull()
            Guan.g37 -> nvwang.isInCar()
            Guan.g41 -> shahuang.isFull() && jiaonv.isFull() && niutou.isFull() && nvwang.isFull() && saman.isInCar() && baoku.isFull() && Zhuangbei.isYandou()
            Guan.g51 -> {
                Zhuangbei.isQiangxi() && saman.isFull()
            }
...
//                }
                var index = if (curG < 8) {
                    var i = heros.indexOf(dijing)
                    if(i<0){
                        i = heros.indexOf(guangqiu)
                        if(i<0 || !dijing.isInCar() || !dijing.isFull()){
                            i = defaultDealHero(heros, arrayListOf( baoku, jiaonv, saman, niutou))
                        }
                    }
                    i
                } else defaultDealHero(heros, arrayListOf(dijing, baoku, jiaonv, saman, niutou, nvwang))
                if (index > -1) {
                    return index
                }
...
//            //dijing赚点钱,满不满都行,沙皇容易抢兵,不在这里下沙皇
            if (shahuang.isInCar()) {
                carDoing.downHero(shahuang)
            }


            //这里不做沙的调整//关卡改变到g21时，会先看地精在不在1号位，不在的话就会下地精，所以  车上无地精先找地精去补一号位，这里会换沙等39


            var index =
                defaultDealHero(heros, arrayListOf(dijing, baoku, jiaonv, saman, niutou, nvwang, zhanjiang))
            if (index > -1) {
//                var hh = heros.get(index)
//                if (!hh!!.isInCar()) {
//                    //dijing赚点钱,满不满都行,沙皇容易抢兵
//                    if (shahuang.isInCar()) {
//                        carDoing.downHero(shahuang)
//                    }
//                }
...
//            }
//
            if (!carDoing.hasOpenSpace()) {//有空位代表之前沙不在1，1和沙都下了。没空位代表之前沙就在1，所以下娇女上地精赚钱
                carDoing.downHero(jiaonv)
            }
            index = defaultDealHero(heros, arrayListOf(shahuang, guangqiu, dijing))//最终只要沙满就行
            if (index > -1) {
                return index
            }
...
//            }
            ...
            var index = defaultDealHero(heros, arrayListOf(saman, guangqiu))
            if (index > -1 && !saman.isFull()) return index
            index = heros.indexOf(huanqiu)
