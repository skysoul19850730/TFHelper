...
class HBZhanNvHeroDoingZiQiangZhanNv : BaseHBHeroDoing(),
...
//es8  es4 ye5   6e0 y80  y50 y56 e04
    ...
    enum class Guan(val des: String? = null) {
        ...
        /**
         * 下亡将，满冰骑，满沙皇，这里点名最多只会下沙皇，所以只需验证冰骑沙皇是否满
         */
        g91("圣骑下，上冰骑"),
...
//        downed.invoke(false)
    ...
    fun isGkOver(g: Guan): Boolean {
        ...
        if (g == Guan.g161) return shahuang.isFull() && wangjiang.isFull()
        if(g == Guan.g151_1)return Zhuangbei.isLongxin() && bingqi.isFull()
        if (g == Guan.g151) return Zhuangbei.isLongxin()
        if (g == Guan.g139) return false
        if (g == Guan.g140) return Zhuangbei.isYandou() && jiaonv.isInCar() && !jiaonv.isFull()
        if(g == Guan.g140_1) return jiaonv.isFull()
//        if (g == Guan.g140) return Zhuangbei.isYandou() && saman.isFull()
        if (g == Guan.g1) return zhanjiang.isFull()
        if (g == Guan.g2) return sishen.isInCar()

        var heroOk = zhanjiang.isFull() && jiaonv.isFull() && saman.isFull()
        if (!heroOk) return false
        return when (g) {
            Guan.g3 -> return sishen.isInCar()

            Guan.g26 -> {
                Zhuangbei.isLongxin() && sishen.isFull() && wangjiang.isInCar()
            }

            Guan.g38 -> {
                Zhuangbei.isLongxin() && sishen.isFull() && shahuang.isFull() && wangjiang.isFull()
            }

            Guan.g91 ->{
                bingqi.isFull() && shahuang.isFull()
            }

            Guan.g101 -> {
                Zhuangbei.isYandou() && sishen.isFull() && shahuang.isFull() && wangjiang.isFull()
            }
            ...
            Guan.g110 -> {
                Zhuangbei.isQiangxi() && sishen.isFull() && shahuang.isFull() && wangjiang.isFull()
            }
...
//        }
        ...
        if (code == VK_NUMPAD3) {
            ...
            if(guankaTask?.currentGuanIndex == 148 || guankaTask?.currentGuanIndex == 149){//监听不到6球得话，就快捷键满娇女
                GlobalScope.launch {
                    guanka = Guan.g140_1
                    waiting = false
                    delay(8000)
...
//                log("战将没满")
            ...
            var index = heros.indexOf(sishen)
            if (index > -1) {
                while (beiNvWang) {
                    delay(20)
                }
            ...
            if ((jiaonv.isInCar() && !jiaonv.isFull()) || (saman.isInCar() && !saman.isFull())) {
                ...
                index = heros.indexOf(guangqiu)
                if (index > -1 && sishen.currentLevel < 3) {
                    return index
                }
            }
...
//            }
            ...
            var fullList = arrayListOf(sishen, jiaonv, saman, zhanjiang)//防止没满
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            var fullList = arrayListOf( sishen, jiaonv, saman, wangjiang,shahuang, zhanjiang)//防止没满
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            return defaultDealHero(heros, arrayListOf(bingqi,shahuang,guangqiu))
        }
            ...
            var fullList = arrayListOf(sishen, shahuang, saman, zhanjiang, wangjiang, jiaonv)
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            var fullList = arrayListOf(sishen, shahuang, saman, zhanjiang, wangjiang, jiaonv, guangqiu, muqiu)
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            var fullList = arrayListOf(zhanjiang, shahuang, jiaonv, wangjiang, sishen, saman)
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            //防止船长点完卡没补满
            var fullList = arrayListOf(zhanjiang, sishen, shahuang, jiaonv, wangjiang, saman)
            var index = defaultDealHero(
                heros,
                fullList
            )
            ...
            if(!jiaonv.isInCar()){
                return heros.indexOf(jiaonv)
            }

        }else if (guanka == Guan.g140_1) {
            return defaultDealHero(heros, arrayListOf(jiaonv,guangqiu))
        }
        else if (guanka == Guan.g151) {
           var index = heros.indexOf(jiaonv)//这里其实就没娇女了，不过，先留着吧(140_1中就会满娇女了)
            if(index>-1){
                return index
            }
            index = heros.indexOf(huanqiu)
            ...
            var fullList = arrayListOf(zhanjiang, sishen, shahuang, jiaonv, wangjiang, saman, guangqiu)
            var index = defaultDealHero(
                heros,
                fullList
            )
