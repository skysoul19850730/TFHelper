...
class HBZhanNvHeroDoingZiQiangZhanNv2 : BaseHBHeroDoing(),
...
//es8  es4 ye5   6e0 y80  y50 y56 e04
    ...
    val kuangjiang = HeroBean("kuangjiang", 90)
...
//        if (g == Guan.g161) return Zhuangbei.isLongxin() && jiaonv.isFull()

//        if (g == Guan.g159_3) return Zhuangbei.getZhuangBei() == needZhuangbei
//        if (g == Guan.g159_2) return jiaonv.isFull()
//        if (g == Guan.g159_1) return bingqi.isFull()

        if (g == Guan.g151) return Zhuangbei.getZhuangBei() == needZhuangbei && bingqi.isFull()
...
//        if (g == Guan.g140) return Zhuangbei.isYandou() && saman.isFull()
        ...
        if (g == Guan.g2) return kuangjiang.isInCar()

        var heroOk = zhanjiang.isFull() && jiaonv.isFull() && saman.isFull()
        if (!heroOk) return false
        return when (g) {
            Guan.g3 -> return kuangjiang.isInCar()

            Guan.g26 -> {
                Zhuangbei.isLongxin() && kuangjiang.isInCar() && wangjiang.isInCar()
            }

            Guan.g38 -> {
                Zhuangbei.isLongxin() && kuangjiang.isInCar() && shahuang.isFull() && wangjiang.isFull()
            }

            Guan.g91 -> {
                bingqi.isFull() && kuangjiang.isFull() && shahuang.isFull()
            }

            Guan.g101 -> {
                Zhuangbei.isYandou() && kuangjiang.isFull() && shahuang.isFull() && wangjiang.isFull()
            }
            ...
            Guan.g110 -> {
                Zhuangbei.isQiangxi() && kuangjiang.isFull() && shahuang.isFull() && wangjiang.isFull()
            }
            ...
            Guan.g161 -> {
                kuangjiang.isFull() && shahuang.isFull() && wangjiang.isFull()
            }
    ...
    suspend fun doOnKeyDown(code: Int): Boolean {
        ...
        if (guankaTask?.currentGuanIndex == 158 || guankaTask?.currentGuanIndex == 159) {
            if (code == VK_NUMPAD3) {
                if(needZhuangbei == Zhuangbei.LONGXIN){
                    needZhuangbei = Zhuangbei.QIANGXI
                }else {
                    needZhuangbei = Zhuangbei.LONGXIN
                }
...
//                log("战将没满")
            ...
            if ((jiaonv.isInCar() && !jiaonv.isFull()) || (saman.isInCar() && !saman.isFull())) {
                ...
                index = heros.indexOf(guangqiu)
                if (index > -1 && kuangjiang.currentLevel < 3) {
                    return index
                }
            }
            ...
            var fullList = arrayListOf( jiaonv, saman, zhanjiang)//防止没满
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            var fullList = arrayListOf(shahuang,  jiaonv, saman, wangjiang, zhanjiang)//防止没满
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            if ((jiaonv.isInCar() && !jiaonv.isFull())||(wangjiang.isInCar() && !wangjiang.isFull())  ||(shahuang.isInCar() && !shahuang.isFull()) || (saman.isInCar() && !saman.isFull())) {
                //g3时，如果萨满或娇女在车上且不满，才用光球。比秒女王升级太快
                index = heros.indexOf(guangqiu)
                if (index > -1 && kuangjiang.currentLevel < 3) {
                    return index
            ...
            var fullList = arrayListOf(kuangjiang, shahuang, saman, zhanjiang, bingqi, jiaonv)
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            var fullList = arrayListOf(kuangjiang, shahuang, saman, zhanjiang, wangjiang, jiaonv)
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            var fullList = arrayListOf(kuangjiang, shahuang, saman, zhanjiang, wangjiang, jiaonv, guangqiu, muqiu)
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            var fullList = arrayListOf(zhanjiang, shahuang, jiaonv, wangjiang, kuangjiang, saman)
//            if (isBaoku()) {
//                fullList.add(bingqi)
//            }
            var index = defaultDealHero(
            ...
            //防止船长点完卡没补满
            var fullList = arrayListOf(zhanjiang, kuangjiang, shahuang, jiaonv, wangjiang, saman)
            var index = defaultDealHero(
                heros,
                fullList
            )
            ...
            if (isGkOver(Guan.g151)) {
                waiting = true
                return -1
            }
        } else if (guanka == Guan.g159_1) {

            if (jiaonv.isInCar()) {
                carDoing.downHero(jiaonv)
            }
            return defaultDealHero(heros, arrayListOf(bingqi, guangqiu))
        } else if (guanka == Guan.g159_2) {

            if (bingqi.isInCar()) {
                carDoing.downHero(bingqi)
            }
            return defaultDealHero(heros, arrayListOf(jiaonv, guangqiu))
        } else if (guanka == Guan.g159_3) {
            ...
            if (index > -1) return index
            index = heros.indexOf(huanqiu)
            if (index > -1 && !Zhuangbei.isLongxin()) {//小翼 烟斗
                return index
            }
            if (isGkOver(Guan.g161)) {
                waiting = true
                return -1
            }
        }
