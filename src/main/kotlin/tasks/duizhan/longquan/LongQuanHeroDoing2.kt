...
class LongQuanHeroDoing2(val renji: Boolean = false) : HeroDoing(-1, FLAG_KEYEVENT) {
    ...
    init {
        ...
        dijing = HeroBean(getRenWuKa(), 3)
//        dijing = HeroBean(getRenWuKa(), 30, needCar = false, compareRate = 0.95)
...
//            delay(1000)
            ...
            if (isGaojiMengyan() || carDoing.openCount()>4) {//大于4时就不停留冰女了，这时只需要上亡将和小丑，然后慢慢满女王了
...
//        }
        ...
        //当龙拳满了，狂龙和霸王有一个在车上了 这时有限满羁绊，比如小丑，冰女或女王占了第三个格子，就下了有限凑羁绊，避免卡钱直接到死都上不去
        if(longquan.isFull() && ( (heroBean == kuanglong && bawang.isInCar()) || (heroBean == bawang && kuanglong.isInCar()))){
            if(xiaochou.isInCar()){
                return xiaochou
            }
...
//            }
            ...
            if(dijing.heroName=="leiqiu" && indexdijing>-1){
                return indexdijing
            }

        } else {

            if(!nvwang.isInCar() || !kuanglong.isInCar() ||!bawang.isInCar() || dijing.heroName !== "wangjiang2") {//有任务卡时 只走原逻辑
                ...
                if(!kuanglong.isInCar() ||!bawang.isInCar()){
                    if(indexxiaolu>-1){
                        return indexxiaolu
                    }
                    if(indexbingnv>-1){
...
//                if(bingnv.isInCar() || xiaochou.isInCar() || bawang.isFull()) {//如果这俩都不在，就只剩4主力+地精了，因为熊猫可能存在，所以不下地精,但如果霸王满了，可以下地精（霸王攻击比龙拳高
                ...
                if(!dijing.isInCar()){
                    if(indexdijing>-1){
                        return indexdijing
                    }else if(indexxiaolu>-1||indexbingnv>-1){
                        return defaultDealHero(heros, arrayListOf(xiaolu,bingnv))
                    }else{
                        return defaultDealHero(heros, arrayListOf(nvwang,guangqiu,kuanglong,bawang))
                    }
                }else{
                    if(xiaochou.isInCar()){
                        return defaultDealHero(heros, arrayListOf(nvwang,guangqiu,kuanglong,bawang,xiaochou,dijing))
                    }else{
                        return defaultDealHero(heros, arrayListOf(xiaochou,nvwang,guangqiu,kuanglong,bawang,dijing))
                    }
                }
            }
//            if (indexdijing > -1) {
